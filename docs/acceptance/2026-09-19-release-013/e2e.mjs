...
if (!LIVE) {
  ...
  /* ===== 4 · HARUTHAI (G048): the fixed Sathorn arrangement never asks, never prices, never blocks; Review shows it as arranged; the emails say so ===== */
  {
    ...
    /* superseded by release 014 (Owner, 19 Sep 2026): no fixed arrangement — the host starts at zero, the stage is open and readiness names it */
    note('haruthai-starts-at-zero', !/Arranged for you|Fixed arrangement|Room A/i.test(h.words) && h.bag === 0 && h.total === 0 && h.state === 'open' && h.need.includes('stage:bkk-stay') && !h.need.some((k) => /^room:/.test(k)), JSON.stringify({ state: h.state, bag: h.bag, total: h.total, need: h.need.slice(0, 3) }));
    await p.context().close();
  }
  ...
  /* ===== 5 · THE CLEAN RESET on the stage: populated state → dry run (nothing changes) → execute → zero → the host's fixed place stands → a device that cached the old trip honours the epoch ===== */
  {
    ...
    const plan2 = await gr('/api/rooms/plan'); let occ2 = 0, fixed2 = 0; for (const units of Object.values(plan2.body.units)) for (const u of units) for (const o of (u.occupants || [])) occ2++;   /* release 014: nothing is fixed */
    ...
    note('reset-executed-zero', run.status === 200 && run.body.dryRun === false && run.body.epoch && run.body.remaining && run.body.remaining.occupancies === 0 && run.body.remaining.holds === 0 && run.body.remaining.kvKeys === 0 && occ2 === 0 && fixed2 === 0 && held2 === 0 && seats2.body.open === true && dry2.body.rooms.occupancies === 0 && dry2.body.seating.holds === 0 && dry2.body.drafts.had === 0 && dry2.body.kv.keys.length === 0, JSON.stringify({ cleared: run.body.rooms.cleared, seats: run.body.seating.cleared, drafts: run.body.drafts.cleared, kv: run.body.kv.deleted, occ: occ2, fixed: fixed2, held: held2, open: seats2.body.open }));
