...
if (!LIVE) {
  ...
  /* ===== 1 · THE MANUAL BOOKING FLOW INSIDE 02 / 06 (T003, Join All): every excursion returns by itself, the context and the Bag survive, one hold per stage ===== */
  {
    ...
    const st0 = await p.evaluate(() => ({ open: SIYL_JOURNEY.open().length, packs: document.querySelectorAll('.p-pack').length, sheets: document.querySelectorAll('[data-scope][aria-pressed="true"]').length, step: /02 \/ 06/.test((document.querySelector('.prep-bar') || {}).innerText || '') }));
    note('flow-start-my-trip', st0.open === 10 && st0.packs === 0 && st0.sheets === 4 && st0.step, JSON.stringify(st0) + ' (no package card — the four sheets, 21 Sep 2026)');
