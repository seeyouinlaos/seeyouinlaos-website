...
/* Edit 5 (Owner, 18 Sep 2026): the arranged room has no Remove / Change; the other Bangkok addresses stay open to the hosts — the fixed unit itself is never on the rail */
const ctlH = await H.evaluate(() => { const s = document.querySelector('#s-bkk-stay'); if (!s) return null; const arranged = s.querySelector('.arr, [data-arranged], .p-arranged') || s; return { removeOnArranged: !!s.querySelector('[data-arranged] [data-rm], [data-arranged] [data-remove], .arr [data-rm], .arr [data-remove]'), penthouseChoice: !!s.querySelector('[data-choose="penthouse"]'), otherChoices: s.querySelectorAll('[data-choose]').length, words: /Arranged for you/i.test(s.innerText) && /Another address, if you prefer/i.test(s.innerText) }; });
note('C-host-no-controls', ctlH && !ctlH.removeOnArranged && !ctlH.penthouseChoice && ctlH.otherChoices >= 1 && ctlH.words, JSON.stringify(ctlH));
...
const leaveH = await api(H, '/api/rooms/leave', { method: 'POST', body: JSON.stringify({ invitationId: 'INV-G049', guestId: 'G049', stage: 'bkk-stay' }) }); note('C-host-engine-release-keeps-arrangement', leaveH.status === 200 && Array.isArray(leaveH.body.released) && leaveH.body.released.length === 0 && leaveH.body.fixed && leaveH.body.fixed['bkk-stay'] && leaveH.body.fixed['bkk-stay'].label === 'A' && leaveH.body.units['bkk-stay/penthouse'][0].taken === 2, leaveH.status + ' released ' + JSON.stringify(leaveH.body.released) + ' fixed ' + JSON.stringify(leaveH.body.fixed));
