/* P0 · REAL DUMMY E2E on the isolated stage worker (PROJECT_MASTER_BRIEF §19–§20). Synthetic guests only (synthetic
   register: T001/T002 one party, T003 another, G048/G049 the synthetic host pair with synthetic codes). No live data touched.
...
const signOut = async (p) => { await p.goto(O + '/your-journey.html', { waitUntil: 'load' }); await p.waitForSelector('header.hd .hb, header.hd #menu-open', { timeout: 20000 }); await p.click('header.hd .hb, header.hd #menu-open'); await p.waitForSelector('[data-account] [data-access-out]', { state: 'visible', timeout: 20000 }); await p.click('[data-account] [data-access-out]'); await p.waitForTimeout(1200); };
...
const completeSteps = async (p, id, name, seats) => {
  await p.goto(O + '/your-journey.html', { waitUntil: 'load' }); await p.waitForTimeout(800); await p.evaluate(() => { SIYL_GUEST.setScope({ all: true }); SIYL_JOURNEY.SEGMENTS.forEach((s) => { if (SIYL_JOURNEY.state(s) === 'open') SIYL_JOURNEY.skip(s.key, true); }); }); await p.waitForTimeout(1500);
  ...
  await p.goto(O + '/about-you.html', { waitUntil: 'load' }); await p.waitForSelector('[data-allergy="no"]', { timeout: 20000 }); await p.click('[data-allergy="no"]'); await p.click('[data-choice="flavor"] [data-pick="Pandan"]'); for (const [k, v] of [['coffeetea', 'Oolong'], ['drink', 'Lime'], ['avoid', 'Nothing'], ['film', 'Film'], ['music', 'Music']]) { const el = await p.$('textarea[data-q="' + k + '"]'); if (el) { await el.fill(v); await el.dispatchEvent('change'); } } const pa = await p.$('[data-photo-ack]'); if (pa && !(await pa.evaluate((e) => e.checked || e.getAttribute('aria-pressed') === 'true'))) await pa.click(); await p.waitForTimeout(1600);
};
...
/* ===== C · the host starts at zero: no arranged item anywhere, Bag USD 0 with nothing chosen, the Penthouse open to the host like anyone, Remove releases ===== */
...
const zero = await H.evaluate(() => ({ items: document.querySelectorAll('[data-arranged-item]').length, words: /Arranged for you|Fixed arrangement/i.test(document.body.innerText), global: typeof window.SIYL_ARRANGED, fixed: typeof SIYL_UNITS.fixed === 'function' ? SIYL_UNITS.fixed('bkk-stay') : false, state: SIYL_JOURNEY.state(SIYL_JOURNEY.SEGMENTS.find((s) => s.key === 'bkk-stay')), mine: SIYL_UNITS.view() ? SIYL_UNITS.view().mine : null }));
note('C-host-bag-zero', sH.bag.length === 0 && sH.total === 0 && sH.bar === 'USD 0', JSON.stringify(sH.bag) + ' bar ' + sH.bar);
note('C-host-starts-at-zero-014', zero.items === 0 && !zero.words && zero.global === 'undefined' && !zero.fixed && zero.state === 'open' && !(zero.mine && zero.mine['bkk-stay']), JSON.stringify(zero));
/* the Bangkok rail offers every address to the host — the Penthouse included — with no arrangement words and nothing to remove before anything is chosen */
const ctlH = await H.evaluate(() => { const s = document.querySelector('#s-bkk-stay'); if (!s) return null; return { choices: [...s.querySelectorAll('[data-choose]')].map((e) => e.getAttribute('data-choose')), remove: !!s.querySelector('[data-rm="bkk-stay"]'), words: /Arranged for you|Fixed arrangement|Another address, if you prefer/i.test(s.innerText) }; });
note('C-host-rail-open-014', ctlH && ctlH.choices.includes('penthouse') && ctlH.choices.includes('u-sathorn-superior-garden') && ctlH.choices.includes('shama-king-studio-balcony') && !ctlH.remove && !ctlH.words, JSON.stringify(ctlH));
/* the host may choose the Penthouse Room A like anyone: the room page offers it, the engine holds it in the host's name, the bag carries the line at its price */
await H.goto(O + '/room.html?stay=sathorn&room=penthouse', { waitUntil: 'load' }); await H.waitForSelector('[data-join="bkk-stay|penthouse|A"]', { timeout: 20000 });
const joinA = await H.$eval('[data-join="bkk-stay|penthouse|A"]', (e) => ({ text: e.textContent.trim(), disabled: !!e.disabled || e.getAttribute('aria-disabled') === 'true' }));
note('C-host-penthouse-open-014', /Choose this room|Join this room/.test(joinA.text) && !joinA.disabled, JSON.stringify(joinA));
await selectRoom(H, 'penthouse', 'A'); const sHp = await state(H);
note('C-host-chooses-like-anyone-014', sHp.bag.length === 1 && /^bkk-stay:penthouse:A:/.test(sHp.bag[0]) && sHp.total > 0 && !!(sHp.mine && sHp.mine['bkk-stay']) && sHp.mine['bkk-stay'].key === 'bkk-stay/penthouse' && sHp.mine['bkk-stay'].label === 'A', JSON.stringify(sHp.bag) + ' ' + JSON.stringify(sHp.mine));
await H.goto(O + '/cart.html', { waitUntil: 'load' }); await H.waitForTimeout(2500); const cartH = await H.evaluate(() => ({ arranged: document.querySelectorAll('[data-arranged-item]').length, words: /Arranged for you|Fixed arrangement/i.test(document.body.innerText), empty: !!document.querySelector('.cart-empty'), removes: document.querySelectorAll('[data-remove]').length, penthouse: /Sathorn Penthouse/.test(document.body.innerText), total: (document.getElementById('cart-total') || {}).textContent }));
note('C-host-cart-real-line-014', cartH.arranged === 0 && !cartH.words && !cartH.empty && cartH.removes === 1 && cartH.penthouse && cartH.total !== 'USD 0', JSON.stringify(cartH));
/* Remove releases it like anyone's: the bag is empty, the engine holds nothing, and an engine leave afterwards finds nothing to release — no arrangement survives */
await cartRemove(H); const sHr = await state(H); note('C-host-remove-releases-014', sHr.bag.length === 0 && sHr.total === 0 && sHr.bar === 'USD 0' && !(sHr.mine && sHr.mine['bkk-stay']), JSON.stringify(sHr.bag) + ' ' + JSON.stringify(sHr.mine));
const leaveH = await api(H, '/api/rooms/leave', { method: 'POST', body: JSON.stringify({ invitationId: 'INV-G049', guestId: 'G049', stage: 'bkk-stay' }) }); note('C-host-engine-nothing-held-014', leaveH.status === 200 && Array.isArray(leaveH.body.released) && leaveH.body.released.length === 0 && leaveH.body.fixed === undefined && !leaveH.body.mine['bkk-stay'] && leaveH.body.units['bkk-stay/penthouse'][0].taken === 0 && leaveH.body.units['bkk-stay/penthouse'][0].reservedFor === null, leaveH.status + ' released ' + JSON.stringify(leaveH.body.released) + ' fixed ' + JSON.stringify(leaveH.body.fixed) + ' penthouse A taken ' + JSON.stringify(leaveH.body.units['bkk-stay/penthouse'][0].taken) + ' reservedFor ' + JSON.stringify(leaveH.body.units['bkk-stay/penthouse'][0].reservedFor));
await completeSteps(H, 'G049', 'GroomTest', { dinner: 'D-B-12' });
await H.goto(O + '/review.html', { waitUntil: 'load' }); await H.waitForTimeout(2500); const revH = await H.evaluate(() => ({ page: location.pathname, arranged: document.querySelectorAll('#arranged [data-arranged-item]').length, words: /Arranged for you|Fixed arrangement/i.test(document.body.innerText), total: document.getElementById('tt').textContent, bar: (document.querySelector('.jbar .jb-t') || {}).textContent })); note('C-host-review', /review/.test(revH.page) && revH.arranged === 0 && !revH.words && revH.total === 'USD 0' && revH.bar === 'USD 0', JSON.stringify(revH));
if (process.env.SHOT) await H.screenshot({ path: path.join(OUT, 'c-host-review.png') });
/* the host's bag stays empty after every step: nothing arranged in advance ever enters it */
...
const plan = await api(H, '/api/rooms/read'); note('C-others-untouched', !plan.body.mine['kmg'] && JSON.stringify(plan.body.summary['bkk-stay/penthouse']).includes('"remainingPlaces":12'), 'penthouse ' + plan.body.summary['bkk-stay/penthouse'].remainingRooms + '/' + plan.body.summary['bkk-stay/penthouse'].remainingPlaces);
...
await A.goto(O + '/review.html', { waitUntil: 'load' }); await A.waitForSelector('#send', { timeout: 20000 }); await A.waitForTimeout(3000); const btn = await A.$eval('#send', (e) => e.textContent); note('E-send-updated-button', btn === 'Send Updated Trip' || btn === 'Send Updated Trip', btn);
await A.click('#send'); await A.waitForSelector('#mailbox[data-mail]:not([data-mail=""])', { timeout: 40000 }); await A.waitForTimeout(1200);
...
const fin = await api(H, '/api/rooms/read'); note('Z-cleanup', fin.body.summary['prewed/noble-courtyard'].remainingPlaces === 2 && fin.body.summary['bkk-stay/u-sathorn-superior-garden'].remainingPlaces === 12 && fin.body.summary['bkk-stay/penthouse'].remainingPlaces === 12, 'noble ' + fin.body.summary['prewed/noble-courtyard'].remainingPlaces + ' · usathorn ' + fin.body.summary['bkk-stay/u-sathorn-superior-garden'].remainingPlaces + ' · penthouse ' + fin.body.summary['bkk-stay/penthouse'].remainingPlaces + ' (nothing is arranged in advance — release 014)');
await b.close();
