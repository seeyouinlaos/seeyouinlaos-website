...
for (const file of ['voyage.html', 'accommodation.html']) {
  for (const w of WIDTHS) {
    ...
    note(shortest + w + ' labels · the Owner\'s seven labels (Lobby · Rooms × 3 · Wedding Ceremony · Wedding Dinner · Coffee & Cake · Breakfast), on the photograph, none colliding, none hidden', info.labels.length === 7 && info.labels.filter((l) => l.zone === 'rooms').length === 3 && info.labels.every((l) => l.visible) && info.overlap.length === 0 && info.inside, info.labels.map((l) => l.text + ' ' + l.font).join(' | ') + (info.overlap.length ? ' OVERLAP ' + info.overlap.join(',') : ''));
...
for (const w of [390, 1280]) {
  ...
  await p.click('.venue-label[data-zone="ceremony"]'); await p.waitForTimeout(600);
  const s1 = await p.evaluate(() => ({ active: document.querySelector('.venue-stage').getAttribute('data-active'), lbl: document.querySelector('.venue-label[data-zone="ceremony"]').getAttribute('aria-pressed'), item: document.querySelector('.venue-item[data-zone="ceremony"]').getAttribute('aria-pressed'), other: document.querySelector('.venue-item[data-zone="dinner"]').getAttribute('aria-pressed'), title: document.querySelector('.venue-title').textContent, live: document.querySelector('.venue-live').textContent, veil: getComputedStyle(document.querySelector('.venue-veil')).opacity, hole: getComputedStyle(document.querySelector('.venue-hole[data-zone="ceremony"]')).opacity, roomsPressed: [...document.querySelectorAll('.venue-label[data-zone="rooms"]')].map((l) => l.getAttribute('aria-pressed')).join() }));
  note('I' + w + ' tap a label on the photograph → that place opens: label and legend pressed, the rest released, the area lit, one-sentence announcement', s1.active === 'ceremony' && s1.lbl === 'true' && s1.item === 'true' && s1.other === 'false' && /Wedding Ceremony/.test(s1.title) && s1.live === 'Wedding Ceremony. Sunday, 28 February 2027 · 15:30.' && Number(s1.veil) > 0.2 && s1.hole === '1' && s1.roomsPressed === 'false,false,false', JSON.stringify(s1));
  await p.click('.venue-label[data-zone="rooms"]'); await p.waitForTimeout(600);
  const s1b = await p.evaluate(() => ({ active: document.querySelector('.venue-stage').getAttribute('data-active'), pressed: [...document.querySelectorAll('.venue-label[data-zone="rooms"]')].map((l) => l.getAttribute('aria-pressed')).join(), holes: [...document.querySelectorAll('.venue-hole[data-zone="rooms"]')].map((h) => getComputedStyle(h).opacity).join(), title: document.querySelector('.venue-title').textContent }));
  note('I' + w + ' the rooms: one place in three houses — one label pressed presses all three, all three areas lit', s1b.active === 'rooms' && s1b.pressed === 'true,true,true' && s1b.holes === '1,1,1' && /rooms/i.test(s1b.title), JSON.stringify(s1b));
  await p.click('.venue-item[data-zone="pool"]'); await p.waitForTimeout(600);
  ...
  note('I' + w + ' a place without a marker (the swimming pool) opens from the legend: its photograph and story, the whole photograph kept, no label pressed', /swimming pool/i.test(s2.title) && s2.img && s2.thumbs === 3 && s2.pressedLabels === 0 && s2.active === '' && Number(s2.veil) < 0.05 && s2.live === 'The swimming pool. Every day of the stay.', JSON.stringify(s2));
  ...
  note('I' + w + ' the gallery: the second photograph replaces the first (cross-fade done, the old one gone), the thumbnail pressed, the file loaded', s3.n === '1' && s3.imgs === 1 && s3.pressed === 'true' && s3.loaded && /053-wedding-dinner-courtyard-villa/.test(s3.src), JSON.stringify(s3));
  ...
  await p.focus('.venue-label[data-zone="coffee"]'); await p.keyboard.press('Space'); await p.waitForTimeout(500);
  ...
  note('I' + w + ' keyboard · a label on the photograph takes focus (visible ring) and Space opens its place', /Coffee & Cake/i.test(k3.title) && k3.outline !== 'none' && k3.active === 'coffee', JSON.stringify(k3));
  ...
  note('I' + w + ' screen reader · the map is described in text, the legend is a labelled list, the detail a labelled region, every photograph has alt text', /On the photograph: Lobby, Rooms, Coffee & Cake · Breakfast, Wedding Ceremony, Wedding Dinner · Poolside\./.test(a11y.figureDesc) && a11y.region === 'region' && a11y.labelled === 'venue-detail-h' && a11y.listLabel === 'Venue map' && a11y.h2 === 'Souphattra Heritage Vientiane' && a11y.alts, JSON.stringify(a11y));
  await b.close();
  /* reduced motion */
  ...
  note('I' + w + ' reduced motion · everything is simply there: the stage, the labels, the outlines, the detail, every story section — nothing moves, nothing is missing', rm.cls && rm.stageIn && rm.labels.length === 7 && rm.labels.every((o) => Number(o) >= 0.6) && rm.img === '1' && /Wedding Dinner/.test(rm.title) && /^0(px)?$/.test(rm.dash) && rm.sections.every((o) => o === '1'), JSON.stringify(rm));
  await rb.close();
}
...
for (const w of [375, 390, 430]) {
  ...
  note('S' + w + ' WebKit · the aerial (' + info.src + ') with the seven labels, no overflow, the detail rendered', /souphattra-aerial/.test(info.src) && info.labels.length === 7 && info.overlap.length === 0 && !info.overflow && /Wedding Dinner/.test(info.title) && errs.length === 0, info.rendered + (errs.length ? ' ' + errs.join('|') : ''));
  await b.close();
}
