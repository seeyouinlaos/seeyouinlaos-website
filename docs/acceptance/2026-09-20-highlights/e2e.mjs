...
if (!LIVE) {
  ...
  /* ===== 2 · THE EXPERIENCES RAIL #highlights: four cards, every image served, the four pages reachable ===== */
  {
    ...
    note('experiences-highlights-rail', rail.map((c) => c.id).join() === 'bkk-suhring,bkk-baanphraya,1872,bkk-cannubi,vte-baron' && served === 5, JSON.stringify(rail.map((c) => c.id)) + ' served ' + served);
    await p.context().close();
  }
