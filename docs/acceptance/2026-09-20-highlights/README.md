...
| Deploy · parity · live-ro (`highlights-live` added) | main `c06bf94` → Workers Build version **`92851aa4`** (12:38:30Z, 20 Sep 2026) · parity 259 / 259 · `assets/highlight.js` and `assets/video/cannubi-card.mp4` served byte-identical · infra freeze intact · the deployed register identical (104 codes, 0 duplicate, 0 auth error) · live-ro **25 / 25** · production read-only smoke at 390 and 1440 (`live-*.jpg`): the Highlights row and its four entries, the rail, the three Highlight pages with their distinction, menus, galleries, the Cannubi clip loading, no price and no add signed out, no overflow, no page error |
| Production transactional state | **real guest activity present** since the code distribution (dry run 12:41Z: 12 room occupancies of 7 guests · 1 seat hold · 2 drafts · 7 KV keys) — **nothing reset**, nothing touched; the Highlights hold no inventory and this deploy changed no inventory (`zero-state-92851aa4.txt`, aggregates only) |
