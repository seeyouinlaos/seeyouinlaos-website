...
const community = (p) => p.evaluate(() => { const c = document.querySelector('#community'); return { state: c ? c.getAttribute('data-community') : null, h2: c && c.querySelector('h2') ? c.querySelector('h2').innerText : '', people: [...document.querySelectorAll('.pf-person')].map((e) => ({ id: e.getAttribute('data-person'), name: e.querySelector('.pf-person-n').innerText, photo: !!e.querySelector('img'), initials: (e.querySelector('.pf-ava i') || {}).innerText || '' })), recent: (document.querySelector('#community .t-b1') || {}).innerText || '', text: c ? c.innerText : '' }; });
...
if (!LIVE) {
  ...
  /* ===== 1 · THE COMMUNITY from the real stage state ===== */
  {
    ...
    note('community-empty-state', c0.state === '0' && /yours could be the first/i.test(c0.text) && c0.people.length === 0, JSON.stringify({ state: c0.state, text: c0.text.slice(0, 120) }));
    ...
    note('community-first-guest-with-portrait', up === 200 && s.clicked && /Sent to Guest Relations/i.test(s.after) && c1.state === '1' && /1 guest has joined so far/.test(c1.h2) && /You are among them/.test(c1.text) && c1.people.length === 1 && c1.people[0].id === 'T003' && /^cleo$/i.test(c1.people[0].name) && c1.people[0].photo === true && /Cleo/.test(c1.recent), JSON.stringify({ up, s, c1: { state: c1.state, h2: c1.h2, people: c1.people, recent: c1.recent } }));
    ...
    note('community-returning-guest-sees-who-joined', /Not joining/i.test(adaSent) && c2.state === '1' && c2.people.length === 1 && c2.people[0].id === 'T003' && c2.people[0].photo === true && !/You are among them/.test(c2.text) && !c2.people.some((x) => x.id === 'T001'), JSON.stringify({ adaSent, c2: { state: c2.state, people: c2.people } }) + ' (a decline is a response, not a joining guest)');
    ...
    note('community-two-guests-initials-and-portrait', sb.clicked && c3.state === '2' && /2 guests have joined so far/.test(c3.h2) && c3.people.length === 2 && c3.people[0].id === 'T002' && c3.people[0].photo === false && c3.people[0].initials === 'B' && c3.people[1].id === 'T003' && c3.people[1].photo === true && !priv, JSON.stringify({ state: c3.state, people: c3.people, priv }));
    const srv = (await api(ben, '/api/community', { method: 'GET' })).body; const keys = srv && srv.guests && srv.guests[0] ? Object.keys(srv.guests[0]).sort().join() : '';
    note('community-api-identity-only', srv && srv.ok && srv.count === 2 && keys === 'guestId,joinedAt,name,photo' && srv.guests.every((g) => /^T\d{3}$/.test(g.guestId)), JSON.stringify({ count: srv && srv.count, keys }));
