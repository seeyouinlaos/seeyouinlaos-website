...
- **Live (17ddc61, automatic Workers Build + Pages, 13 Sep 23:16):** parity **255/255** (the accepted list + `experience.html`, `assets/gallery.js`, `assets/experience-galleries.js` and all 190 experience photographs) · experience walk **72/72 on Worker and Pages** (`experience-walk-worker.json`, `-pages.json`) · smoke 8/8 both · crawl 0 HTTP failures / 0 console errors on both (one Pages run aborted a 700 KB card image mid-navigation; the file serves 200 and the re-run is clean) · **iPhone-class WebKit pass on the Worker 21/21** (`experience-iphone.mjs`: discovery rails, gallery pans 1→2→3→2, lazy frames, accordion, gate, selection → In your journey, reload, no duplicate, Your Journey, Review & Send, Remove, Dib and Lao Derm galleries). Automation cannot synthesise a real touch pan in WebKit: the pan is reproduced as the track's own native horizontal scroll, and the track is verified as finger-pannable (overflow-x, scroll-snap x, touch-action not none), the same mechanism as the accepted discovery rails.

### Sühring — FINAL OWNER DECISION (13 Sep 2026, late evening): USD 180 PER PERSON, in the total
The Afternoon Tea pattern, applied: Sühring is a FLAT product of the one calculation source (`assets/pricing.js` `FLAT.suhring`: 180 · Restaurant · basis "USD 180 per person · a table requested through Guest Relations · not a confirmed reservation"). The place page's action carries a **Participating guests** stepper (default: the whole party once the invitation is known; never above the party, never below one) and writes ONE party-level line `{ id:'suhring', price:180, qty:<participants>, request:true, exp:'bkk-suhring' }`; the state reads **In your journey · 2 guests · USD 360**, Remove reverses it. Your Journey shows the line with the same stepper and the amount; Review & Send shows "USD 180 per person × 2 guests", the request wording and USD 360 in the total; the send payload carries `RESTAURANT REQUEST (USD 180 per person × 2 participating; to be arranged through Guest Relations; not a confirmed reservation)`. No date or lunch time is asked; no booking engine; no "confirmed" or "guaranteed" anywhere (pinned). The 13-image folder (9 curated), the About record, the hours and the map are unchanged.
Coverage: `test/experiences.test.mjs` +5 (per-person pricing by the calculation source · multiplication and total · no duplicate + reload · Your Journey / Review wiring · wording guard) — `npm test` 229/229 · gates PASS · experience walk **74/74** (1 guest = USD 180, 2 = USD 360, stepper stops at the party, reload, duplicate impossible, Your Journey 360 → 180 → 360, Review & Send USD 360, Remove → USD 0).
- **Live (e955f15, automatic Workers Build + Pages, 14 Sep 00:14):** parity 255/255 · experience walk **74/74 on Worker and Pages** · smoke 8/8 both · **iPhone-class WebKit 21/21** on the Worker (1 guest USD 180 → party of two USD 360 · In your journey · 2 guests · USD 360 · reload · no duplicate · Your Journey and Review & Send at USD 360 · Remove).

### 002 — SEATING UX UPGRADE (Owner instruction 14 Sep 2026) → `docs/acceptance/2026-09-14-seating-ux/`
The plan drawn as a room · guest labels A/B | aisle | D/E/F × 1–10 and A1–A25 / B1–B25 (never a ledger id in the words) · two-step booking (tap → sticky summary → CONFIRM → SEAT CONFIRMED) · CHANGE SEAT with the old seat authoritative until the engine answers · a downloadable PDF seat confirmation (no QR, no barcode) · the hosts' front centre by role. The seating engine is untouched. Released at `36ae09d` through the automatic Workers Build and Pages, parity 257/257, the real booking flow proven on production with INV-002 and INV-001 and the ledger restored exactly. Record, evidence and the adversarial review: the 14 Sep README.

### Owner markup, verified against production first (14 Sep 2026, midday)
Five iPad markups from 13 Sep, each checked on the live Worker and Pages before touching anything: **Lao Traditional Dress** (no "Blue") — ALREADY CORRECT, 0 active occurrences, guarded by `test/guest-testing.test.mjs` and gate P4 · **C86 Business Class** without "1 + 1 seating" and its explanatory claims — ALREADY CORRECT, guarded · **05 About You · REQUIRED** — ALREADY CORRECT (Travel Documents stay optional/later per the DOCS decision) · **Morning Alms-Giving main image** (one male guest kneeling alone) — STILL WRONG → `temple/takbat-couple-giving-novice.jpg` (Owner's `007 _Couple_Monk.jpeg`, Drive `1bJruhMOv9_tGr7m3k89o9LWokFyDFH6x`) · **Morning Alms-Giving secondary image** (bowl-and-garland close-up) — STILL WRONG → `temple/takbat-couple-offering-bowl.jpg` (Owner's `002_Couple_Monk.jpeg`, Drive `1w4kuxmtNfxuBst9CONdFLbxdJekgZiOv`). The series is physically filed under Drive folder 170 (Dib Bangkok) and is mapped to its correct wedding role — `assets/images/ASSET-MAP.md`, section "14 SEP Morning Alms-Giving". Released at `34b7b42` (automatic build), parity **259/259**, both origins serve the new frames and answer 404 for the retired files; crawl 0 failures / 0 broken images; the section rendered at 390 / 700 / 834 / 1440 on both origins, cover frames, no distortion, no page overflow.
