...
/* CART = CART · REVIEW = REVIEW — and ONE PRICE SOURCE: a line saved at the superseded C86 105 (the Owner's Edit 2 screenshot) shows 85 and counts 85 */
await p.evaluate(() => { const b = JSON.parse(localStorage.getItem('siyl.bag') || '[]'); const c = b.find((x) => x.id === 'c86'); if (c) c.price = 105; localStorage.setItem('siyl.bag', JSON.stringify(b)); });
...
note('T6 the cart · C86 saved at 105 reads USD 85 on the line and USD 85 per person beneath it; the total is the sum of authoritative amounts', c86line && c86line.amt === 'USD 85' && /USD 85 per person/.test(c86line.text) && !/USD 105/.test(cartT) && totals.c86 === 85 && totals.cart === money(totals.engine) && totals.engine === totals.sum, JSON.stringify(totals));
