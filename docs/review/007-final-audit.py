#!/usr/bin/env python3
"""007 — FINAL SOURCE-TRUTH AUDIT · generator for docs/review/007-final-source-truth-audit.txt
...
Nothing on the website is changed; where sources conflict, the conflict is recorded, never resolved.
  python3 docs/review/007-final-audit.py <ops-master.xlsx> <site-facts.json> <extract-run.json>
"""
...
# ------------------------------------------------------------------ the four moments
issue('The Wedding (03) · Wedding public page · seat tickets', 'Temple Ceremony · 08:00 – 12:00 · Wat Ong Teu, Vientiane (seat ticket: 08:00)',
      ...
      'Master_Timeline DAY 07 rows 05:30–06:15 · Overview Day 08 · design-system master §18', 'CONFLICT', 'Venue matches (Wat Ong Teu). The start time differs: website 08:00 · locked programme 09:00 · the sheet times the alms giving at 05:30 at the hotel. NEEDS OWNER DECISION.')
...
issue('The Wedding (03) · Wedding public page', 'Vow Ceremony · 16:30 · Souphattra Heritage Vientiane', 'Master_Timeline 28.02: 16:20 Bride walks down the aisle · 16:30–17:00 Wedding ceremony · Souphattra Heritage Vientiane; design-system master §18: VOW CEREMONY · 16:30', 'Master_Timeline DAY 07 16:30–17:00', 'MATCH')
issue('The Wedding (03) · Wedding public page · seat tickets · seating plan', 'Wedding Dinner · 19:30 · Souphattra Heritage Vientiane · poolside (seat ticket 19:30)', 'Master_Timeline 28.02: 18:30–19:00 Go to wedding dinner venue · 19:00–19:15 Grand entrance · 19:15–21:30 Wedding Dinner (Appetizer & main course) · Souphattra Heritage Vientiane; design-system master §18: WEDDING DINNER · 19:30 · Souphattra Vientiane Hotel', 'Master_Timeline DAY 07 19:00–21:30 · design-system master §18', 'CONFLICT', 'Time: website 19:30 (as the locked programme) · sheet grand entrance 19:00, dinner 19:15. Venue name: website "Souphattra Heritage Vientiane" · locked programme "Souphattra Vientiane Hotel" · sheet "Souphattra Heritage Vientiane" / vendor "Souphattra Hotel Vientiane". NEEDS OWNER DECISION on the time; the venue name follows the sheet.')
...
issue('The Wedding · dress code', 'Temple Ceremony: Lao Traditional Dress · Coffee & Cake, Vow Ceremony, Wedding Dinner: Black Tie · travelling days: Resort Wear', 'Master_Timeline 28.02: guests "Black Tie" from 15:30; alms giving "Denim, White"; Bangkok days "Elegant Resort Wear" / "Cocktail Attire"', 'Master_Timeline DAY 07 · DAY 02–03 dress column', 'CONFLICT', 'The wedding-day Black Tie matches. The sheet dresses the morning "Denim, White"; the website says Lao Traditional Dress for the temple (Owner decision 13 Sep: no "blue"). NEEDS OWNER DECISION.')
...
issue('Journeys · Your Journey · Transport · travel pass · sent journey', 'C86 · high-speed train · Kunming Railway Station 10:15 → Lijiang Railway Station 13:44 · 04 March 2027 · direct · 3 h 29 m · about 527 km · Business Class · USD 105 per person',
      'Overview Day 12-01: "Train C642 · 16:39 → 21:06 · Direct · 4 hr 27 min · USD 105 per person · BUSINESS CLASS · 1+1 seating layout · PRIORITY TICKETING"; Accommodation_Details: China Train · 105 · 4 available; Owner truth 15 Sep 2026: Train C86 · 10:15 → 13:44 · direct 3h29 · Business Class · USD 105 (register/data.mjs kmg-ljg comment)', 'Overview Day 12-01 · Accommodation_Details · Owner correction 15 Sep 2026', 'OWNER DECISION OVERRIDE', 'The sheet still names C642 16:39 → 21:06; the Owner replaced it with C86 10:15 → 13:44 (and struck the "1 + 1 seating" claim, 13 Sep). The amount USD 105 matches the sheet.')
...
# ------------------------------------------------------------------ counts
classes = {'MATCH': 0, 'CONFLICT': 0, 'SOURCE MISSING': 0, 'OWNER DECISION OVERRIDE': 0}
...
a11y = json.load(open(ROOT / 'docs/review/.007-a11y-check.json')) if (ROOT / 'docs/review/.007-a11y-check.json').exists() else []
out = []
w = out.append
w('SOURCE-TRUTH AUDIT')
...
w('STALE / RETIRED SOURCE ONLY: 6 (the sheet still carries C642 16:39 → 21:06, the room reservations for Bride & Groom / Family, Khop Chai Deu, Le Padaek, Parkson Supermarket and the Overview\'s USD 90 Penthouse rate — none is active on the website)')
w('')
w('NOTE: the website\'s rooms are audited in full below (section K), the prices in full (section J), the experiences in full (section L). Classification MATCH means the website says what the sheet says; CONFLICT means the sources disagree and nobody chose; SOURCE MISSING means the sheet carries no value for the claim; OWNER DECISION OVERRIDE means a documented Owner decision replaces the sheet value. Every ACTION is NO CHANGE — REVIEW ONLY.')
w('')
...
open(ROOT / 'docs/review/007-final-source-truth-audit.txt', 'w').write('\n'.join(out) + '\n')
summary = dict(claims=total_claims, match=match_total, conflict=conflict_total, missing=classes['SOURCE MISSING'], overrides=classes['OWNER DECISION OVERRIDE'], secret='PASS' if run['secretHits'] == 0 else 'FAIL', completeness='PASS' if not run['leftover'] and not run['failures'] else 'FAIL')
json.dump(summary, open(ROOT / 'docs/review/.007-audit-summary.json', 'w'))
