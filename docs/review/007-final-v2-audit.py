#!/usr/bin/env python3
"""007 — FINAL V2 SOURCE-TRUTH AUDIT · generator for docs/review/007-final-v2-source-truth-audit.txt (003: after the Owner patch of 15 Sep 2026 and the venue experience)
...
Nothing on the website is changed; where sources conflict, the conflict is recorded, never resolved.
  python3 docs/review/007-final-v2-audit.py <ops-master.xlsx> <site-facts.json> <extract-run.json>
...
issue('The venue stage · labels ON the photograph', 'Three labels sit on real areas: 05 Wedding Dinner · Poolside (the pool terrace), 06 Swimming pool, 07 Courtyard garden', 'Owner decision 15 Sep 2026: Wedding Dinner = Poolside, Run A = Poolside; the pool and the lawn are the photograph itself (Heritage_0631, Heritage_0640, caption (8))', 'Owner decision 15 Sep 2026 · Drive 003 - Hotel - Pool & Garden', 'OWNER DECISION OVERRIDE', 'The dinner marker follows the Owner\'s poolside truth; the pool and the garden are visible in the source photograph. No compass direction is used.')
issue('The venue stage · labels WITHOUT a position', 'Lobby · Rooms (×3 in the Owner\'s concept) · Coffee & Cake / Breakfast · Wedding Ceremony are in the venue legend with their real photography and their story, but carry no marker on the photograph', 'The Owner-marked layout (which building is the lobby, which houses are the rooms, where Coffee & Cake / Breakfast and the Wedding Ceremony take place on the aerial) was not among the delivered materials (Edit 2(1).zip holds six review screenshots; no annotated aerial in any upload or Drive folder)', 'Owner instruction 003, PART 4 ("The Owner-marked layout determines approximate placement")', 'SOURCE MISSING', 'NEEDS OWNER DECISION: the marked layout. Rule 003 / PART 19 forbids inferring building purpose from a photograph, so nothing is drawn for these four until the Owner marks the aerial; adding them is a data change in assets/venue-data.js (area + anchor), no code. AFFECTS PUBLIC RELEASE (release gate "labels correctly mapped").')
...
a11y = json.load(open(ROOT / 'docs/review/.007-v2-a11y-check.json')) if (ROOT / 'docs/review/.007-v2-a11y-check.json').exists() else []
out = []
w = out.append
w('SOURCE-TRUTH AUDIT · FINAL V2 (003 — after the Owner patch of 15 Sep 2026 and the venue experience)')
...
open(ROOT / 'docs/review/007-final-v2-source-truth-audit.txt', 'w').write('\n'.join(out) + '\n')
...
json.dump(summary, open(ROOT / 'docs/review/.007-v2-audit-summary.json', 'w'))
