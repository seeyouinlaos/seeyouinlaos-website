#!/usr/bin/env python3
"""007 — FINAL V3 DELTA · docs/review/007-final-v3-delta.md (final V2 → final V3)
Compares the final V2 review (007-final-v2-text.txt · 007-final-v2-source-truth-audit.txt) with the
final V3 (007-final-v3-text.txt · 007-final-v3-source-truth-audit.txt) against final V2: the unique text lines that appeared
and disappeared, grouped by what they belong to, and the source-truth classifications that changed. Concise —
the corpus is never repeated here.  python3 docs/review/007-final-v3-delta.py"""
import re, pathlib, collections, datetime
ROOT = pathlib.Path(__file__).resolve().parents[2]
rd = lambda f: (ROOT / 'docs/review' / f).read_text()
pre, v2 = rd('007-final-v2-text.txt'), rd('007-final-v3-text.txt')
def lines(t):
...
out = []
w = out.append
w('# 007 — FINAL V3 DELTA (final V2 → final V3)'); w('')
w('Generated ' + datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d %H:%M UTC') + ' by docs/review/007-final-v3-delta.py.'); w('')
w('FINAL V2:  docs/review/007-final-v2-text.txt (MAIN SHA ' + re.search(r'MAIN SHA: (\w+)', pre).group(1) + ') · docs/review/007-final-v2-source-truth-audit.txt')
w('FINAL:     docs/review/007-final-v3-text.txt (MAIN SHA ' + re.search(r'MAIN SHA: (\w+)', v2).group(1) + ') · docs/review/007-final-v3-source-truth-audit.txt'); w('')
...
# classifications
pa, pb = rd('007-final-v2-source-truth-audit.txt'), rd('007-final-v3-source-truth-audit.txt')
def issues(t):
    out = {}
...
(ROOT / 'docs/review/007-final-v3-delta.md').write_text('\n'.join(out) + '\n')
