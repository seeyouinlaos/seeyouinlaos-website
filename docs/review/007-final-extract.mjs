/* 007 — FINAL FULL-WEBSITE TEXT EXTRACTION · generator for docs/review/007-final-full-website-text.txt
...
const OUT = path.join(ROOT, 'docs/review/007-final-full-website-text.txt');
...
function add(route, state, source, context, parts) {
  ...
  if (!/^PUBLIC · DEFAULT|^TEMPLATE|^GENERATED/.test(state)) stateCount++;
...
for (const stay of Object.keys(ROOMS)) for (const r of ROOMS[stay].rooms || []) {
  await step('room ' + stay + '/' + r.slug, async () => { await go('room.html?stay=' + stay + '&room=' + r.slug, 390); roomRoutes++; await S('/room?stay=' + stay + '&room=' + r.slug, 'PUBLIC · DEFAULT · signed out', 'room.html · assets/rooms-data.js', ROOMS[stay].name + ' · ' + (r.name || r.slug) + ' · the room page with the physical rooms of the category'); });
}
/* transport detail pages */
for (const id of ['train', 'mu9646', 'c86', 'return']) await step('transport ' + id, async () => { await go('transport.html?id=' + id, 390); await S('/transport?id=' + id, 'PUBLIC · DEFAULT · signed out', 'transport.html · assets/transport-data.js · assets/travelpass.js', 'the transport detail page with the ticket (not selected)'); });
...
for (const [label, party, gid, mine, events] of [
  ['Temple Ceremony seat ticket (a guest)', partyP, 'G001', { ceremony: { G001: 'C-R-04-02' }, dinner: {} }, ['ceremony']],
  ...
  ['Temple Ceremony · the Groom (front centre only)', partyH, 'G049', { ceremony: {}, dinner: {} }, ['ceremony']],
...
for (const leg of TP.ORDER) for (const [state, price, cls] of [['selected', TP.LEGS[leg].id === 'mu9646' ? 275 : { train: 100, c86: 105, return: 200 }[leg], 'business'], ['sent', TP.LEGS[leg].id === 'mu9646' ? 155 : { train: 100, c86: 105, return: 200 }[leg], 'economy-flexible'], ['confirmed', TP.LEGS[leg].id === 'mu9646' ? 275 : { train: 100, c86: 105, return: 200 }[leg], 'business']]) {
...
add('generated · the ticket templates (every field and its dynamic values)', 'TEMPLATE', 'assets/seatpass.js · assets/travelpass.js', 'what varies on the tickets', [{ name: 'pdf', lines: [
  ...
  'Stub: the QR code (carries: SEE YOU IN LAOS / SEAT TICKET <reference> / <first name> / <event> / Sunday, 28 February 2027 · <time> / Seat <label> · <words> | <fixed words> / CONFIRMED | FRONT CENTRE) · <reference> SYL-TC-<label>-XXXX | SYL-WD-<label>-XXXX | SYL-TC-FC-XXXX · CONFIRMED | FRONT CENTRE · TEMPLE CEREMONY | WEDDING DINNER · 28 FEB 2027 · <time> · SCAN AT THE DOOR',
...
function renderAll() {
  ...
  const out = [];
  out.push('007 — FINAL FULL-WEBSITE TEXT EXTRACTION · see you in laos.');
...
fs.writeFileSync(path.join(ROOT, 'docs/review/.007-a11y-check.json'), JSON.stringify(a11yIssues, null, 2));
