...
After the fixes: suite 304/304, release check PASSED, stage E2E 49/49 re-run, 48 screenshots re-shot (0 overflow).

**Confirming pass** (thread 01a0b248, branch diff after defa525): F-1 and F-2 confirmed resolved; one further finding:

| # | Codex finding | Class | Fix | Pin |
|---|---|---|---|---|
| F-3 | A Save whose PUT was acknowledged (R2) but whose read-back GET failed never recorded R2 or the stored keys as the base (`done()` unreached). An answer typed meanwhile was then pushed against R1, refused with the device's own earlier answer, and overwritten by the merge as a "double-sided" change. | VALID | The acknowledged PUT's revision and keys become the base immediately, before any read-back; `done()` keeps `dirty` true when something was typed while the request was in flight. | "CODEX FINAL-3": PUT ok, read-back fails, an answer typed in between → pushed against R2, no 409, the newest answer stays; dirty stays true for an edit that outran its request |

After F-3: suite 305/305, release check PASSED, stage E2E 49/49 re-run, 48 screenshots re-shot (0 overflow). The closing Codex pass on d92442d: `CODEX REVIEW: DEFERRED — TEMPORARILY UNAVAILABLE` (usage limit; window reopens 08:26 on 18 Sep 2026) — to be run against the latest implementation before any release (see the acceptance README).
