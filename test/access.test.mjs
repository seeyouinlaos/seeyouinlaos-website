...
test('ACCESS · the pages: the bag, the tickets, the room and transport planning pages and every private step hand over when nobody is signed in; The Journey is public editorial with its private fragments removed and the way in; the public close CTA is "Open your invitation" → the invitation page; every public page loads the gate', () => {
  ...
  assert.match(j, /<title>The Journey · See You In Laos<\/title><script src="assets\/invite-early\.js"><\/script>/); assert.match(j, /<main><h1>The Journey<\/h1>/);
  ...
  for (const f of ['journeys.html', 'experience.html', 'tea.html', '1872.html', 'experiences.html', 'voyage.html']) assert.match(src(f), /<script src="assets\/invite-early\.js"><\/script>/, f + ' decides before the first paint');
