...
test('ACCESS · the pages: the bag, the tickets, the room and transport planning pages and every private step hand over when nobody is signed in; The Journey is public editorial with its private fragments removed and the way in; the public close CTA is "Open your invitation" → the invitation page; every public page loads the gate', () => {
  ...
  assert.equal((j.match(/<p class="pp" data-private>/g) || []).length, 5, "release 012 · 23 Sep 2026: the Riverside Hotel is retired, so one private amount fragment fewer"); assert.equal((j.match(/<p class="pb" data-private/g) || []).length, 9); assert.equal((j.match(/<div class="vars" data-private data-/g) || []).length, 6); assert.equal((j.match(/<div class="psel" data-private data-select/g) || []).length, 1); assert.equal((j.match(/<button class="add" data-private data-add=/g) || []).length, 3); assert.match(j, /<p class="jstat" id="jstat" data-private><\/p>/);
