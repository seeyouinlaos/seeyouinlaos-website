...
test('ACCESS · the pages: the bag, the tickets, the room and transport planning pages and every private step hand over when nobody is signed in; The Journey is public editorial with its private fragments removed and the way in; the public close CTA is "Open your invitation" → the invitation page; every public page loads the gate', () => {
  ...
  assert.equal((j.match(/<p class="pp" data-private>/g) || []).length, 6, "release 012: the Riverside Hotel card carries its amount as a private fragment; 21 Sep 2026: the Guest House its USD 0"); assert.equal((j.match(/<p class="pb" data-private/g) || []).length, 10); assert.equal((j.match(/<div class="vars" data-private data-/g) || []).length, 6); assert.equal((j.match(/<div class="psel" data-private data-select/g) || []).length, 1); assert.equal((j.match(/<button class="add" data-private data-add=/g) || []).length, 3); assert.match(j, /<p class="jstat" id="jstat" data-private><\/p>/);
