...
test('ACCESS · signed in: require() runs at once, private links keep their targets, the header names the guest with Your Journey and Sign out; on the invitation page a signed-out require() opens the prompt in place', { skip: !hasSTM && 'run with --experimental-vm-modules' }, async () => {
  ...
  assert.equal(inserted[0].attrs['data-state'], 'in'); assert.match(inserted[0].innerHTML, /Signed in as Peggy/); /* TO-00188 */ assert.match(inserted[0].innerHTML, /href="your-journey\.html" data-access-nav="trip">My Trip</); assert.match(inserted[0].innerHTML, /data-access-nav="profile">My Profile</, 'the account navigation lives in the menu drawer (Owner, 18 Sep 2026 · Aman header)'); assert.match(inserted[0].innerHTML, /data-access-out>Sign out</);
...
test('ACCESS · the pages: the bag, the tickets, the room and transport planning pages and every private step hand over when nobody is signed in; The Journey is public editorial with its private fragments removed and the way in; the public close CTA is "Open your invitation" → the invitation page; every public page loads the gate', () => {
  ...
  assert.match(j, /<title>The Journey · See You In Laos<\/title>(?:<script src="assets\/i18n\/siyl-i18n\.js(?:\?v=[0-9a-f]{8})?"><\/script>\n?)?<script src="assets\/invite-early\.js(?:\?v=[0-9a-f]{8})?"><\/script>/); assert.match(j, /<main><h1>The Journey<\/h1>/);
  ...
  assert.equal((j.match(/<p class="pp" data-private>/g) || []).length, 5, "release 012 · 23 Sep 2026: the Riverside Hotel is retired, so one private amount fragment fewer"); assert.equal((j.match(/<p class="pb" data-private/g) || []).length, 7, 'Window 007: the train package basis (TO-00503) and the MU9646 fare range line (TO-00551) are removed'); assert.equal((j.match(/<div class="vars" data-private data-/g) || []).length, 6); assert.equal((j.match(/<div class="psel" data-private data-select/g) || []).length, 1); assert.equal((j.match(/<button class="add" data-private data-add=/g) || []).length, 3); assert.match(j, /<p class="jstat" id="jstat" data-private><\/p>/);
  ...
  assert.equal((j.match(/<a class="vw" data-cta-swap href="(?:transport|room)\.html/g) || []).length, 5, 'the view links into private planning swap their label signed out (23 Sep 2026: one fewer, the Riverside is retired)');
  ...
  /* Window 007 · PRQ-07a-03: the dormant generic select branch of experience.html is deleted — every selectable place is a Highlight, whose booking section (SIYL_HIGHLIGHT.html) carries the private price, the private #selbox and the way in; signed out, paint() empties the box */
  assert.match(src('experience.html'), /<div data-private><span class="k">Price<\/span>/); assert.match(src('experience.html'), /if \(isHighlight\) h \+= SIYL_HIGHLIGHT\.html\(x\);/); assert.doesNotMatch(src('experience.html'), /x\.select\.price/, 'no generic select branch any more');
  { const hl = src('assets/highlight.js'); assert.match(hl, /'<p class="price" data-private>' \+/); assert.match(hl, /'<div id="selbox" data-private><\/div>' \+/); assert.match(hl, /data-private-cta href="invitation\.html\?open=1">Open your invitation<\/a>/); assert.match(hl, /if \(!signedIn\(\)\) \{ box\.innerHTML = ''; box\.removeAttribute\('data-sig'\); return; \}/); }
  assert.match(src('tea.html'), /<p class="price" data-private>USD 180<\/p>/); assert.match(src('tea.html'), /<div id="selbox" data-private><\/div>\n<a class="cta" data-private-cta href="invitation\.html\?open=1">Open your invitation<\/a>/);
  assert.match(src('1872.html'), /<p class="price" data-private>USD 180<\/p><p class="per" data-private>One table for two guests<\/p>/);
  /* EDIT 9 (Owner, 26 Sep 2026): the Sühring, 1872 and Marsilea sections are deleted from Experiences — no amount left there at all */
  assert.doesNotMatch(src('experiences.html'), /<h2>(Sühring|Champagne Afternoon Tea at 1872|Marsilea Spa)<\/h2>|USD \d/, 'the deleted sections and their amounts are gone');
  assert.match(src('voyage.html'), /Sangkhathan · optional<span data-private> · USD 15 per person<\/span>/); assert.equal((src('voyage.html').match(/data-cta-swap href="journeys\.html#j-wedstay"/g) || []).length, 2);
  assert.equal((src('accommodation.html').match(/<a class="a-more" data-cta-swap href="room\.html\?stay=/g) || []).length, 2);   /* U Sathorn, the residence (16 Sep 2026) — the Riverside Hotel was retired 23 Sep 2026 (release 012), the Sathorn Penthouse deleted 24 Sep 2026 (Edit 6), Shama Yen-Akat deleted 24 Sep 2026 */
  ...
  assert.doesNotMatch(src('accommodation.html'), /room=shama-king-studio-balcony|<h3>Shama/, 'Shama Yen-Akat is deleted (Owner, 24 Sep 2026)');
  ...
  assert.match(src('assets/invite.mjs'), /data-access-nav="trip">My Trip<\/a><a href="' \+ hrefOf\('profile\.html'\) \+ '" data-access-nav="profile">My Profile<\/a>' \+ \(window\.SIYL_DRAFT \? '<button type="button" class="a-macct-save" data-access-save>Save my progress<\/button>' : ''\) \+ '<button type="button" class="a-macct-out" data-access-out>Sign out<\/button>/, 'signed in: My Trip · My Profile · Sign out in the drawer');
...
test('EDIT 3 · Harudot is a café; the dress references share one 3:4 card, covered from the top, never stretched', () => {
  /* the taxonomy (Owner, 22 Sep 2026): Harudot is a CAFÉ — one card, its two days (23.02 and 07.03); never an experience */
  assert.match(src('assets/experiences.js'), /\{ id: 'bkk-harudot', category: 'cafe', roles: \['cafe'\], visits: \[\{ day: 3, date: '2027-02-23'[^\]]*\{ day: 15, date: '2027-03-07'/);
