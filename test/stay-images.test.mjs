/* ACCOMMODATION IMAGE QUALITY (Owner, 16 Sep 2026): one source map for the Bangkok imagery — the hero, the card and the
   ...
   it; THE HOUSES carries U Sathorn and Shama with the map's images; every referenced file exists. The Sathorn Penthouse
   is deleted (Owner, 24 Sep 2026 · Edit 6): no map entry, no room, no card — its files on disk are unreferenced. */
...
test('ONE SOURCE MAP · hero, card and gallery per property, files present, no cross-property leakage, every rail reads the card from the map', () => {
  ...
  assert.ok(M && M.uSathorn && M.shamaYenAkat); assert.equal(M.sathornPenthouse, undefined, 'the Sathorn Penthouse is deleted (Edit 6)');
  ...
  assert.equal(pe, undefined, 'no Penthouse room record'); assert.equal(R.length, 2, 'Bangkok is exactly U Sathorn and Shama');
  for (const k of Object.keys(M)) { const m = M[k]; if (!m || typeof m !== 'object') continue; assert.doesNotMatch(JSON.stringify(m), /images\/(journey\/)?penthouse\//, k + ' references no Penthouse frame'); }
...
test('THE HOUSES · U Sathorn and Shama each with the map\'s image, no Penthouse card; no missing image on any card', () => {
  ...
  /* one card fewer since the Riverside Hotel was retired (Owner, 23 Sep 2026), another since the Sathorn Penthouse was deleted (Owner, 24 Sep 2026 · Edit 6) */
