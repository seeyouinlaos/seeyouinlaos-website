...
test('THE DATA · three Highlight tables with distinction, editorial line, house, menu, contact and a request product; the amounts are the Owner\'s rounded USD prices of the houses\' menu prices; pairings, supplements and caviar are never products', () => {
  ...
  assert.deepEqual(s.highlight.contact.address, ['No. 10, Yen Akat Soi 3', 'Chongnonsi, Yannawa', '10120 Bangkok, Thailand']); assert.equal(s.highlight.contact.phone, '+66 2 107 2777'); /* TO-03312: one phone grouping everywhere */ assert.equal(s.highlight.contact.email, 'reservation@restaurantsuhring.com');
  ...
  assert.match(s.highlight.menuNote, /beverages are not included/); /* TO-02711 */ assert.match(s.highlight.menuNote, /10% service charge and 7% VAT/);
  ...
  assert.match(b.highlight.menuNote, /plus 10% service charge and government tax/); assert.match(b.highlight.menuNote, /are not included here/); /* TO-02788 */
  assert.deepEqual(b.practical.hours, ['Pre-dinner drink 17:00 – 18:00 · the outdoor terrace', 'Dinner 18:00 – 23:00 · Friday to Tuesday']); assert.equal(b.practical.dress, 'Women in elegant attire and proper footwear; men in long trousers and closed shoes — sleeveless shirts are not permitted for men.'); /* TO-02806 */
  assert.equal(b.highlight.contact.phone, '+66 2 659 9000'); /* TO-03311 */ assert.equal(b.highlight.contact.email, 'mobkk-baanphraya@mohg.com');
  assert.equal(P.FLAT.baanphraya.price, 114); assert.match(P.FLAT.baanphraya.basis, /THB 3,800/); assert.match(P.FLAT.baanphraya.basis, /^USD 114 per person · plus 10% service charge and government tax/, /* TO-01456 */ 'the USD amount never claims to include the house\'s charges');
  const c = byId['bkk-cannubi'];
  assert.equal(c.highlight.distinction, 'One MICHELIN Star · The MICHELIN Guide Thailand 2026'); assert.match(c.highlight.line, /Andrea Susto/); assert.equal(c.highlight.house, 'An intimate dining room on the L Floor of Dusit Thani Bangkok, with a cellar of more than 350 wine labels.'); /* TO-02853 */ assert.match(c.teaser, /Cannubi hill of Barolo/); /* TO-02848 */
  ...
  assert.equal(c.highlight.menuNote, 'This is the house’s autumn card; the menu in March may differ. The same set menu is served to everyone at the table. Wine and non-alcoholic pairings are not included here. Please tell Guest Relations early about any allergies or dietary preferences.'); /* TO-02855: the charges moved out of the note — the product basis carries them */ assert.match(P.FLAT.cannubi.basis, /plus 10% service charge and government tax/);
  assert.deepEqual(c.practical.hours, ['Wednesday to Sunday', 'Lunch 12:00 – 14:30 (last order 14:00)', 'Dinner 18:00 – 22:00 (last order 21:30)']); /* TO-02869: 'L Floor' removed from the hours (the house line names it) */ assert.equal(c.practical.dress, 'Smart casual — please no T-shirts, shorts or sandals.'); /* TO-02872 */
  assert.equal(c.highlight.contact.phone, '+66 2 200 9000'); /* TO-03310 */ assert.equal(c.highlight.clip, 'assets/video/cannubi-card.mp4');
...
test('THE BOOKING GRAMMAR · one line per house with the chosen menu\'s price, a request; put replaces (a menu change), never duplicates; removal reverses; the line reads as an experience — never as travel — in the record', () => {
  ...
  assert.match(hl, /data-sel-state="confirm"/); assert.ok(hl.includes('It is sent to us with your trip as a request; Guest Relations confirms the table with you. You can remove it from My Bag at any time.'), 'TO-03036: the confirm sheet says it is a request'); assert.doesNotMatch(hl, /nothing is held until you confirm/i);
...
test('THE NAVIGATION · a Highlights row in the one menu with Aman first, the Experiences rail #highlights with the four, both footers alike; nothing removed from Experiences', () => {
  ...
  /* PRQ-07a-06: the canonical product '1872', priced for the table; the old 'tea1872' line is still READ as the same line */
  assert.match(src('tea.html'), /select:\{id:'1872',legacy:\['tea1872'\],unit:'table'/);
  assert.match(src('tea.html'), /product:\{name:'Champagne Afternoon Tea at 1872',meta:'One table for two guests'/);
  { const w = page({ auth: PEGGY }); const t = w.SIYL_PRICE.FLAT['1872']; assert.equal(t.price, 180); assert.equal(t.name, 'Champagne Afternoon Tea at 1872'); assert.equal(t.meta, 'One table for two guests'); assert.equal(t.unit, 'experience', 'no bogus unit:true flag'); }
});
...
test('THE SATHORN CARD INSET · the stay card\'s text is one column behind one inset token; the shared carousel\'s heading rule is out-ranked; no per-element side margin remains (regression)', () => {
  ...
  assert.match(j, /'<div class="pcbody">'\+/); assert.match(j, /Details<\/a>'\+\s*'<\/div><\/div>'/); /* B4: “See the rooms” → “Details” */
