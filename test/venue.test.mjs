...
test('ASSETS · the base is the Owner\'s aerial as built (bytes pinned, 2560 × 1440 and the 4:5 crop of the same frame); every photograph exists at every declared width and format; each maps to the Drive manifest', () => {
  ...
  assert.equal(DATA.base.source.drive, '1VIz9oIZDOUlktJD7pase7e4UilhvsO9j');
  const base = manifest.entries.find((e) => e.driveId === DATA.base.source.drive);
  ...
  for (const z of DATA.zones) for (const p of z.photos) {
    ...
    assert.ok(p.source && p.source.folder && p.source.file, z.id + ' photograph has a source');
    ...
    if (p.source.drive) { const m = manifest.entries.find((e) => e.driveId === p.source.drive); assert.ok(m && m.selected, z.id + ' ' + p.source.file + ' is a selected manifest entry'); }
  }
