...
import { page, json, plain, src, roomsFetch, doState, PEGGY, STEFFIE, HARUTHAI, SUTHEP } from './sandbox.mjs';
...
test('CART · a preset holds the guest\'s own places, keeps a manual choice, and never selects a full or reserved room', async () => {
  const rooms = new Rooms(doState());
  const w = await livePage(PEGGY, rooms);
  const J = w.SIYL_JOURNEY, ST = w.SIYL_STAY, B = w.SIYL_BAG, P = w.SIYL_PRICE;
  await ST.select('prewed', 'heritage-executive');
  const plan = J.fullExperience();
  assert.ok(plan.kept.includes('prewed'), 'a manual choice is protected');
  assert.ok(plan.add.every((it) => it.qty === undefined || it.qty === 1));
  assert.ok(!plan.add.some((it) => it.room === 'souphattra-presidential'), 'never a reserved room');
  for (const it of plan.add) { if (it.room) { const r = await ST.select(P.windowOf(it.id), it.room); assert.equal(r.ok, true); } else { it.qty = 1; B.put(it); } }
  assert.equal(J.open().length, 0);
  assert.equal(ST.line('prewed').room, 'heritage-executive');
  assert.ok(B.get().every((x) => x.qty === 1));
});
