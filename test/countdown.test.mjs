...
test('THE MODEL · wedding first: the two targets, the values from the clock (never written), the states before · today · journey · after, never negative, a day is a day whatever the hour', () => {
  ...
  assert.deepEqual(c0.wedding, { state: 'before', n: 159, unit: 'days', eyebrow: 'The wedding · in', word: null, tail: 'Sunday, 28 February 2027 · Vientiane, Laos' });
  ...
  assert.deepEqual(c1.wedding, { state: 'before', n: 7, unit: 'days', eyebrow: 'The wedding · in', word: null, tail: 'Sunday, 28 February 2027 · Vientiane, Laos' });
  /* under way, before the wedding */
  const c2 = plain(C.countdowns(at(2027, 2, 24)));
  assert.deepEqual(c2.journey, { state: 'journey', n: 4, unit: 'of 16 days', eyebrow: 'The journey · day', word: null, tail: '21 February – 8 March 2027 · Thailand · Laos · China' });
  ...
  assert.equal(c4.journey.n, 11); assert.equal(c4.journey.unit, 'of 16 days');
  ...
  const js = src('assets/community.js'); assert.doesNotMatch(js, /\b15[0-9]\b\s*(days|,)|\b16[0-9]\b\s*days/, 'no written count');
});
...
test('THE PAGE · the wedding block first and primary, the journey second and secondary; the numerals keep their final width (the invisible final value beneath the live digits); the word states carry no zero; the section reveals once — a re-render never replays; reduced motion is the final state at once', () => {
  ...
  assert.match(h, /^<section class="prep-sec pf-count" id="countdown" data-countdown="before" data-wedding="before" data-journey="before" data-cd-reveal aria-label="The wedding in 159 days · The journey begins in 152 days">/);
  assert.ok(h.indexOf('data-cd="wedding"') < h.indexOf('data-cd="journey"'), 'the wedding first');
  assert.match(h, /<div class="pf-cd pf-cd-primary" data-cd="wedding" data-cd-state="before"><p class="t-l1 pf-cd-eyebrow">The wedding · in<\/p><p class="pf-num" data-count-to="159" data-key="cd-wedding"><span class="pf-num-v pf-num-fixed"><span class="pf-num-size" aria-hidden="true">159<\/span><span class="pf-num-live">0<\/span><\/span><span class="pf-num-u">days<\/span><\/p><p class="t-l1 mute pf-cd-tail">Sunday, 28 February 2027 · Vientiane, Laos<\/p><\/div>/);
  assert.match(h, /<div class="pf-cd pf-cd-secondary" data-cd="journey" data-cd-state="before"><p class="t-l1 pf-cd-eyebrow">The journey begins · in<\/p><p class="pf-num" data-count-to="152" data-key="cd-journey"><span class="pf-num-v pf-num-fixed"><span class="pf-num-size" aria-hidden="true">152<\/span><span class="pf-num-live">0<\/span>/);
  ...
  const after = C.countdownHtml(at(2027, 3, 5)); assert.match(after, /data-wedding="after"/); assert.match(after, /<span class="pf-num-v">Married<\/span>/);
  ...
  /* the wire: the section joins the observer; without one (or under reduced motion) it is in at once; the count-up staggers the two counts */
  const js = src('assets/community.js'); assert.match(js, /cd = root\.querySelector\('\[data-cd-reveal\]'\)/); assert.match(js, /if \(cd\) \{ cd\.classList\.add\('is-in'\); shown\.cd = true; \}/); assert.match(js, /delay = key === 'cd-journey' \? 450 : key === 'cd-wedding' \? 120 : 0/);
