...
test('8 · the overview reflects selections, holds and the waiting list — nothing is arranged for anyone, the Bag is read and never totalled here (Owner, 19 Sep 2026)', async () => {
  ...
  assert.deepEqual(Object.keys(HU).filter((k) => k.startsWith('bkk-stay/')), ['bkk-stay/u-sathorn-superior-garden'], 'one Bangkok alternative');
  for (const k of ['bkk-stay/u-sathorn-superior-garden']) assert.ok(HU[k].length > 0 && HU[k].every((u) => u.reservedFor === null && u.taken === 0), k + ': no room is anyone\'s before a booking');
});
