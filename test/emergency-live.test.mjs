...
test('EMAIL · the journey is stored first with a submission id, then Guest Relations and the guest are mailed through the provider; the answer carries both message ids and no code', async () => {
  ...
  try {
    ...
    assert.ok(owner.htmlContent && guest.htmlContent, 'both emails carry the CI HTML'); assert.match(guest.htmlContent, /<img src="https:\/\/seeyouinlaos-website\.suthep-hrg\.workers\.dev\/assets\/brand\/see-you-in-laos-email\.png\?v=[0-9a-f]{8}" width="280" height="65" alt="see you in laos\."/, 'the official wordmark (Owner, 25 Sep 2026)'); assert.doesNotMatch(guest.textContent + guest.htmlContent, /2026-09-16T|INV-G001|(?<!SYL-)G001\b|ledger|engine/); assert.match(guest.textContent, /^Sent on \d+ \w+ 2026$/m, 'the date only (B3)');
