...
test('THE JOURNEY IN NUMBERS · every number counted from the canonical data on the page; nothing invented; a category that cannot be counted is absent', () => {
  ...
  assert.equal(by.stays.n, Object.keys(M).filter((k) => k !== '_taxonomy').length); /* one house fewer since the Riverside was retired (Owner, 23 Sep 2026), another since the Sathorn Penthouse was deleted (Owner, 24 Sep 2026 · Edit 6), another since Shama Yen-Akat was deleted (Owner, 24 Sep 2026) */ assert.equal(by.stays.n, 6); assert.equal(M.sathornPenthouse, undefined, 'the Sathorn Penthouse is not counted'); assert.equal(M.shamaYenAkat, undefined, 'Shama Yen-Akat is not counted');
  ...
  assert.deepEqual({ r: by.restaurants.n, c: by.cafes.n, b: by.bars.n, m: by.museums.n, t: by.temples.n }, { r: 14, c: 11, b: 6, m: 2, t: 2 }, 'the real totals of the corrected dataset: 14 restaurants (Petits Plats and Cam On in, Time Space / Moo Yoo / Kaogee out) · 11 cafés (Dior Café and LV Café two places since 22 Sep 2026) · 6 bars & nightlife (BARON, Selene, Firefly in; Sona once) · 4 museums (Wat Si Saket gone) · 2 temples & stupas (Wat Ong Teu, Pha That Luang)');
