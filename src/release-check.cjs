   ...
   an asset. */
{
  ...
  if (!/guestSeats: 50, left: 20, right: 30, fixed: 2/.test(led) || !/ceremony: \{ rows: 10, perRow: \{ L: 2, R: 3 \}, guestSeats: 50, fixed: \['BRIDE', 'GROOM'\]/.test(led)
    || !/guestSeats: 50, top: 25, bottom: 25, totalPeople: 50/.test(led) || !/perSide: 25, guestSeats: 50, totalPeople: 50, lastNumber: 26, retired: \{ T: \[13\], B: \[13\] \}, id: \/\^D-\(\[TB\]\)-\(0\[1-9\]\|1\[0-9\]\|2\[0-6\]\)\$\//.test(led)
    || /guestSeats: 48|perSide: 24/.test(led) || /dinner:\s*\{[^}]*fixed:/.test(led)) bad.push('the seating capacity contract is not the Owner geometry (ceremony 50 + BRIDE/GROOM front centre; dinner 50 bookable — A1–A12, A14–A26 · B1–B12, B14–B26, no 13, nothing renumbered, nothing fixed)');
  ...
  gate('P11', 'E/F/G: explicit eligibility, protected confirmation, geometry-free seating ledger',
    bad.length === 0, bad.length ? bad.join(' · ') : 'PAIR/NONE/unresolved only · GR token gate with constant-time compare · the ceremony geometry uploaded, the fifty dinner chairs generated by code without a 13 · token never in assets or git');
}
 ...
 * future geometry; the dinner has 50 seats (A1–A12, A14–A26 · B1–B12, B14–B26 — Owner override, 25 Sep 2026), nothing renumbered, no substitute such as 12A. */
{
  ...
  const dinner = []; for (const side of ['T', 'B']) for (let n = 1; n <= 26; n++) { const lab = L.label('D-' + side + '-' + two(n)); if (lab) dinner.push(lab); }
  const want = [...Array(26).keys()].map((i) => i + 1).filter((n) => n !== 13);
  const exact = JSON.stringify(dinner) === JSON.stringify(want.map((n) => 'A' + n).concat(want.map((n) => 'B' + n)));
  const engine = /export const NEVER_SEAT_NUMBER = 13;/.test(seatSrc) && /dinner:\s*\{ guestSeats: 50, top: 25, bottom: 25/.test(seatSrc) && /if \(n !== NEVER_SEAT_NUMBER\) out\.push\(/.test(seatSrc);
  ...
  gate('R13', 'The number 13 is never a guest-facing seat number (D-29, permanent)',
    ...
    bad.length ? 'seat 13 reachable: ' + bad.join(', ') : !exact ? 'the dinner labels are not A1–A12, A14–A26 · B1–B12, B14–B26' : !engine ? 'src/seating.js lost NEVER_SEAT_NUMBER, the 50-seat capacity or the generator that skips 13' : subst ? 'a substitute seat label (12A/12B) appears in ' + subst : '50 dinner seats, no 13 anywhere, no substitute, nothing renumbered');
}
